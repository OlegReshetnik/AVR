#include "serial_lib.h"
	...
	{
		uint16_t timeout = SERIAL_TIMEOUT;
		while( !serial_rx_ready() & timeout ) timeout--;
		if( !timeout ) return -1;
		return (uint16_t)UDR0;
	}

	uint8_t serial_tx( uint8_t data )
	{
		uint16_t timeout = SERIAL_TIMEOUT;
		while( !serial_tx_ready()  & timeout ) timeout--;
		if( !timeout ) return 0;
		UDR0 = data;
		return 1;
	}
	...
	uint8_t serial_tx( uint8_t data )
	{
		...
		while( ( head == tx_tail ) & timeout ) timeout--; // Ждать место
		if( !timeout ) return 0;
