...
#include "flash_kfreq_lib.h"

const uint8_t s1[] PROGMEM = "◊¿—“Œ“Õ€… œ–»¡Œ–";
const uint8_t s2[] PROGMEM = "›À≈ “–ŒÕ 11 pro";

const uint8_t m1[] PROGMEM = "Œ—Õ. œ–Œ√–¿ÃÃ€";
const uint8_t m2[] PROGMEM = " À¿–  œ–Œ√–¿ÃÃ€";
const uint8_t m3[] PROGMEM = "ƒŒœ. œ–Œ√–¿ÃÃ€";
const uint8_t * const main_menu[] PROGMEM = { m1, m2, m3, (uint8_t *)0 };
...
int main(void)
{
    ...
    while(1)
    {
		switch( menu_get( MainMenu(), MENU_PGM ) )
		{
			...
			case 1:
			{
				ret_flash = menu_get( kpg_menu(), MENU_PGM );
				if( ret_flash != MENU_RETURN )
				{
					ret_i = menu_get( razdel_knames( ret_flash ), MENU_PGM );
					if( ret_i != MENU_RETURN ) play_programm( ret_flash, ret_i, PROG_KPGM );
					else sound2_beep( 880, 500 );
				}
				else sound2_beep( 880, 500 );
			}
